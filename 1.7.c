...
int main()
{
    ...
    while ((Y == 0) && (No == 0))
    {
        ...
        else if ((step == 1) && (r->prev->j != r->j+1))//we can go right -> go right (of course, only if we have not come from there)
        {
            push(r,r->i,r->j+1);
            r = r->next;
        }
        else if ((step == 2) && (r->prev->i != r->i+1))//We can go down -> go down
        {
            push(r,r->i+1,r->j);
            r = r->next;
        }
        else if ((step == 3) && (r->prev->j != r->j-1))//We can go left -> go left
        {
            push(r,r->i,r->j-1);
            r = r->next;
        }
        else if ((step == 4) && (r->prev->i != r->i-1))//We can go up -> go up
        {
            push(r,r->i-1,r->j);
            r = r->next;
        }
